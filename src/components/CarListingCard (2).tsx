...
export const CarListingCard: React.FC<CarListingCardProps> = ({
  ...
  // Photo switching (arrows, counter, swipe) is an ADMIN-ONLY control.
  // Public visitors just see the cover photo and open the car to see the
  // rest of the gallery inside the detail view.
  const canBrowsePhotos = isAdminLoggedIn && !!car.photos && car.photos.length > 1;
  ...
  // ADMIN-ONLY: the exact day / month / year this car was listed.
  // Public visitors never see it — only a logged-in admin does.
  const listedOnLabel = (() => {
    if (!car.createdAt) return null;
  ...
  return (
    ...
    >
      <div>
        ...
        <div 
          className={`relative h-56 sm:h-64 overflow-hidden bg-slate-950 select-none ${isSoldAndHidden ? 'cursor-default' : 'cursor-pointer'}`}
          onClick={() => { if (!isSoldAndHidden) onSelect(car); }}
        ...
        >
          ...
          {/* PHOTO SWITCHING ARROWS — ADMIN ONLY.
              Public cards show the cover photo only; the full gallery is
              still available to everyone inside the car detail view. */}
          {canBrowsePhotos && (
            <>
              {/* Left Arrow Button */}
              <button
        ...
        <div className="p-4 sm:p-5 space-y-3">
          ...
          {/* ADMIN ONLY: date this car was listed (day / month / year) */}
          {isAdminLoggedIn && listedOnLabel && (
            <div className="flex items-center gap-1.5 text-[11px] font-bold bg-amber-50 border border-amber-200 text-amber-800 px-2.5 py-1.5 rounded-xl">
              <Calendar className="w-3.5 h-3.5 text-amber-600 shrink-0" />
              <span className="uppercase tracking-wide text-amber-600/80">
                {lang === 'am' ? 'የተለጠፈበት ቀን' : 'Listed on'}:
              </span>
              <span className="font-mono text-amber-900">{listedOnLabel}</span>
              <span className="ml-auto text-[9px] font-black uppercase bg-amber-200/70 text-amber-900 px-1.5 py-0.5 rounded">
                Admin
              </span>
            </div>
          )}
          ...
          <h3 
            ...
            className={`text-base sm:text-lg font-bold text-slate-900 leading-snug line-clamp-2 transition-colors ${
              isSoldAndHidden ? '' : 'hover:text-blue-700 cursor-pointer'
            }`}
          >
            {lang === 'am' && car.titleAm ? car.titleAm : car.title}
          </h3>

          {/* Tech Spec Chips (Transmission, Fuel, Color) — hidden once sold */}
          {!isSoldAndHidden && (
      ...
      <div className="p-4 sm:p-5 pt-0 flex items-center gap-2">
        {isSoldAndHidden ? (
          <span
            className="flex-1 bg-slate-200 text-slate-500 text-xs font-bold py-2.5 px-4 rounded-xl flex items-center justify-center gap-1.5 cursor-not-allowed select-none"
            title={lang === 'am' ? 'መኪናው ተሸጧል — ዝርዝር መረጃ ተሰውሯል' : 'Car sold — details hidden'}
          >
            <Lock className="w-3.5 h-3.5" />
            <span>{lang === 'am' ? 'ተሸጧል — ዝርዝር የለም' : 'Sold — No Further Details'}</span>
          </span>
        ) : (
          <>
            <button
