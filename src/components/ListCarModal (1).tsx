...
export const ListCarModal: React.FC<ListCarModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    //
    // Sellers now have a choice of proof: a typed transaction reference,
    // an uploaded receipt (PDF/photo/screenshot), or both. We only reject
    // the submission if BOTH are missing — we no longer force the
    // transaction reference to be typed when a receipt file is already
    ...
    if (!isAdminLoggedIn) {
      ...
      if (!hasRef && !hasReceiptFile) {
        setErrorMsg(lang === 'am'
          ? 'ክፍያ ሳይፈጽሙ መኪናዎን መዘርዘር አይችሉም! እባክዎ የ50 ብር ክፍያ ፈጽመው የግብይት ቁጥር ያስገቡ ወይም የክፍያ ደረሰኝ (ፒዲኤፍ/ፎቶ/ስክሪንሽት) ያያይዙ (አንዱ ብቻ በቂ ነው)።'
          : 'Payment is strictly required! No one can list their car without paying the 50 ETB listing fee. Please either enter your transaction reference number OR attach a payment receipt (PDF, photo, or screenshot) — one of the two is enough.');
        return;
      }

      // CRITICAL CHECK: WHAT IF HE USED TWO TIMES IN ONE TRANSACTION?
      // Only meaningful when a reference number was actually provided —
      // a receipt-only submission has nothing to compare against here.
      if (hasRef) {
    ...
    // Save used transaction reference to prevent reuse (only when one was
    // actually provided — a receipt-only submission has no ref to record).
    // Wrapped in try/catch so a storage hiccup here can never block the
    // actual listing submission below.
      ...
      submitHandler({
        ...
        sellerContact: {
          ...
          notes: !isAdminLoggedIn 
            ? `[PAID 50 ETB] ${listingTxnRef.trim() ? `Ref: ${listingTxnRef.trim()} ` : ''}via ${listingPaymentMethod.toUpperCase()}${receiptScreenshot ? (receiptFileType === 'pdf' ? ` (PDF Attached: ${receiptFileName})` : ' (Screenshot/Photo Attached)') : ''} | ${sellerNotes}` 
