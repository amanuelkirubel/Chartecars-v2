...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      console.error('Failed to persist cars to localStorage, retrying without receipt attachments', err);
      try {
        const slimmed = cars.map((c) =>
          c.paymentDetails?.receiptScreenshot
            ? { ...c, paymentDetails: { ...c.paymentDetails, receiptScreenshot: undefined } }
            : c
        );
        localStorage.setItem(STORAGE_KEY_CARS, JSON.stringify(slimmed));
        setToastMessage(
          lang === 'am'
            ? 'ማከማቻ ቦታ ስላለቀ የክፍያ ደረሰኝ ፎቶ/ፒዲኤፍ ማስቀመጥ አልተቻለም፣ ነገር ግን የመኪናው ዝርዝር ተቀምጧል።'
            : 'Storage space is full, so the receipt photo/PDF for one or more listings could not be saved — but the listing itself was saved safely.'
        );
        setTimeout(() => setToastMessage(null), 6000);
      } catch (err2) {
        console.error('Failed to persist cars to localStorage even after stripping receipts', err2);
        setToastMessage(
          lang === 'am'
  ...
  const handleAddCar = (newCarData: Omit<CarListing, 'id' | 'createdAt' | 'views'>) => {
    ...
    if (newCar.status === 'pending') {
      setToastMessage(lang === 'am' 
        ? 'የመኪናዎ መረጃ ገብቷል! የ50 ብር ክፍያው በአስተዳዳሪው ሲረጋገጥ ወዲያው ይለጠፋል።' 
        : 'Listing submitted! Awaiting 50 ETB admin payment verification before going live.');
    } else {
