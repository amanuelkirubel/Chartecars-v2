...
// Public marketing reference codes (C1, C2, C10...) used for YouTube/TikTok posts.
// Assigned once per car, oldest listing first, and never reused/reassigned
// after that — even if an earlier car is later deleted.
...
// One-time migration helper: fills in listingCode for any car that doesn't
// have one yet (e.g. data saved before this feature existed), ordering by
// createdAt so the earliest-listed car becomes C1.
...
export default function App() {
  ...
  // ---------------------------------------------------------------
  // Make the phone/browser back button close whichever modal is open
  // instead of exiting the whole site (see hooks/useModalBackClose.ts
  // for why this was needed).
  // ---------------------------------------------------------------
  useModalBackClose(!!selectedCar, () => setSelectedCar(null));
  useModalBackClose(isListCarModalOpen, () => setIsListCarModalOpen(false));
  useModalBackClose(isCarAdminModalOpen, () => setIsCarAdminModalOpen(false));
  useModalBackClose(isCarFavoritesModalOpen, () => setIsCarFavoritesModalOpen(false));
  useModalBackClose(isDownloadModalOpen, () => setIsDownloadModalOpen(false));
  useModalBackClose(isAboutModalOpen, () => setIsAboutModalOpen(false));
  useModalBackClose(isPaymentModalOpen, () => setIsPaymentModalOpen(false));
  useModalBackClose(isSearchModalOpen, () => setIsSearchModalOpen(false));

  // Persistence Effects
  //
  // IMPORTANT: `cars` can include large base64 payment-receipt attachments
  // (PDFs/photos/screenshots sellers upload). Browsers cap localStorage at
  // roughly 5-10MB per origin, so JSON.stringify(cars) can occasionally
  // exceed that quota and localStorage.setItem() throws a
  // QuotaExceededError. Previously that exception was unhandled, which
  // crashed the whole app to a blank screen right after a seller
  // submitted a listing — even though their transaction reference had
  // already been marked "used" a moment earlier in ListCarModal, so a
  // retry was then wrongly rejected as a duplicate with no listing ever
  // showing up in the Admin Portal to approve.
  //
  // Fix: never let this write crash the app. If the full payload is too
  // big to store, retry once with heavy receipt attachments stripped out
  // (keeping every other detail — make/model/price/seller info/txn ref —
  // intact) so the listing itself is never silently lost.
  useEffect(() => {
    // Tier 1: store everything.
    const tryStore = (payload: CarListing[]): boolean => {
      try {
        localStorage.setItem(STORAGE_KEY_CARS, JSON.stringify(payload));
        return true;
      } catch (err) {
    ...
    if (tryStore(cars)) return;

    // Tier 2: drop the heavy base64 receipt attachments, keep everything
    // else (make/model/price/seller info/transaction reference).
    const withoutReceipts = cars.map((c) =>
      c.paymentDetails?.receiptScreenshot
        ? { ...c, paymentDetails: { ...c.paymentDetails, receiptScreenshot: undefined } }
        : c
    );
    if (tryStore(withoutReceipts)) {
      setToastMessage(
        lang === 'am'
          ? 'ማከማቻ ቦታ ስላለቀ የክፍያ ደረሰኝ ፎቶ/ፒዲኤፍ ማስቀመጥ አልተቻለም፣ ነገር ግን የመኪናው ዝርዝር ተቀምጧል።'
          : 'Storage was full, so the receipt photo/PDF could not be kept — but every listing itself was saved safely.'
      );
      setTimeout(() => setToastMessage(null), 6000);
      return;
    }

    // Tier 3: keep only the cover photo of older listings. The 12 newest
    // listings keep their full gallery; everything older keeps one photo.
    // This is what stops a listing from being silently lost once several
    // sellers have posted from the same browser.
    const trimmed = withoutReceipts.map((c, i) =>
      i < 12 ? c : { ...c, photos: c.photos.slice(0, 1) }
    );
    if (tryStore(trimmed)) {
      setToastMessage(
        lang === 'am'
          ? 'ማከማቻ ሙሉ በመሆኑ የቆዩ ዝርዝሮች ፎቶዎች ተቀንሰዋል፤ ሁሉም መኪኖች ግን ተቀምጠዋል።'
          : 'Storage was full, so extra photos on older listings were trimmed — all listings themselves were saved.'
      );
      setTimeout(() => setToastMessage(null), 6000);
      return;
    }

    // Tier 4: last resort — one photo per listing for everyone.
    const minimal = withoutReceipts.map((c) => ({ ...c, photos: c.photos.slice(0, 1) }));
    if (tryStore(minimal)) {
      setToastMessage(
        lang === 'am'
          ? 'ማከማቻ ሙሉ ስለሆነ ለእያንዳንዱ መኪና አንድ ፎቶ ብቻ ተቀምጧል። ሁሉም ዝርዝሮች ግን ደህና ናቸው።'
          : 'Storage was full, so only one photo per listing was kept. No listing was lost.'
      );
      setTimeout(() => setToastMessage(null), 6000);
      return;
    }
    ...
    setToastMessage(
      lang === 'am'
        ? 'መኪናውን ማስቀመጥ አልተቻለም (ማከማቻ ሙሉ ነው)። እባክዎ የቆዩ ዝርዝሮችን ያጽዱ ወይም ደጋግመው ይሞክሩ።'
        : 'Could not save your listing — device storage is full. Please free up space or try again.'
    );
  ...
  const handleAddCar = (newCarData: Omit<CarListing, 'id' | 'createdAt' | 'views'>) => {
    ...
    if (newCar.status === 'pending') {
      setToastMessage(lang === 'am' 
        ? `የመኪናዎ መረጃ ገብቷል! የ${LISTING_FEE_ETB} ብር ክፍያው በአስተዳዳሪው ሲረጋገጥ ወዲያው ይለጠፋል።` 
        : `Listing submitted! Awaiting ${LISTING_FEE_ETB} ETB admin payment verification before going live.`);
    } else {
